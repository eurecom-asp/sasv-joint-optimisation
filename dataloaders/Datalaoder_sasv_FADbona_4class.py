...
class SASV_Trainset(Dataset):
    def __init__(self, cm_embd, partition, spk_meta_all, spk_meta_original, data_dir):
        ...
        self.spk_meta = spk_meta_all
        self.spk_meta_original = spk_meta_original
    ...
    def __getitem__(self, index):
        ...
        if randnumber == 0:  # target bonafide
            ...
            spk = random.choice(list(self.spk_meta_original.keys()))
            if len(self.spk_meta_original[spk]["spoof"]) == 0:
                while True:
                    spk = random.choice(list(self.spk_meta_original.keys()))
                    if len(self.spk_meta_original[spk]["spoof"]) != 0:
                        break
            enr = random.choice(self.spk_meta_original[spk]["bonafide"])
            tst = random.choice(self.spk_meta_original[spk]["spoof"])
            
        elif randnumber == 3: # nontarget spoof
            spk, ze_spk = random.sample(self.spk_meta_original.keys(), 2)
            enr = random.choice(self.spk_meta_original[spk]["bonafide"])
            tst = random.choice(self.spk_meta_original[ze_spk]["spoof"])
...
def get_trnset(
    cm_embd_trn: Dict, partition_flag: Dict, spk_meta_all: Dict, spk_meta_original: Dict, database: Dict,
) -> SASV_DevEvalset:
    return SASV_Trainset(
        cm_embd=cm_embd_trn, partition=partition_flag, spk_meta_all=spk_meta_all, spk_meta_original=spk_meta_original, data_dir=database,
    )
